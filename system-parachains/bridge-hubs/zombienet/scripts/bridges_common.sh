...
function upgrade_runtime() {
    ...
    local runtime_para_id=$3
    local runtime_para_endpoint=$4
    local wasm_runtime_path=$5
    echo "  calling setCodeWithoutChecks:"
    echo "      relay_url: ${relay_url}"
    echo "      relay_chain_seed: ${relay_chain_seed}"
    echo "      runtime_para_id: ${runtime_para_id}"
    echo "      runtime_para_endpoint: ${runtime_para_endpoint}"
    echo "      wasm_runtime_path: ${wasm_runtime_path}"
    echo "      params:"

    # 1. read runtime wasm
    # WASM=`xxd -p $wasm_runtime_path | tr -d '\n'`

    # 2. generate data for Transact (System::set_code_without_checks)
    local tmp_output_file=$(mktemp)
    generate_hex_encoded_call_data "set-code-without-checks" "${runtime_para_endpoint}" "${tmp_output_file}" "${wasm_runtime_path}"
    local hex_encoded_data=$(cat $tmp_output_file)

    # 3. trigger governance call
    send_governance_transact "${relay_url}" "${relay_chain_seed}" "${runtime_para_id}" "${hex_encoded_data}" 200000000 12000
}
