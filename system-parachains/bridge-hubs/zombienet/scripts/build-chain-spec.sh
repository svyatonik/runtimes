...
if [ $BUILD_SPEC -eq 1 ]; then
    ...
    echo "=== EGS: $POLKADOT_PARACHAIN_BINARY_PATH export-genesis-state --chain $PLAIN_CHAIN_SPEC">>/tmp/my
    $POLKADOT_PARACHAIN_BINARY_PATH export-genesis-state --chain $PLAIN_CHAIN_SPEC
elif [ $EXPORT_GENESIS_WASM -eq 1 ]; then
    echo "=== EGW: $RUNTIME_WASM_BLOB">>/tmp/my
    xxd -p $RUNTIME_WASM_BLOB | tr -d '\n' | awk '{print "0x"$1}'
else
    echo "=== RUN: $BINARY $BINARY_ARGS">>/tmp/my
    $BINARY $BINARY_ARGS
fi
