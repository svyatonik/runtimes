...
function setCodeWithoutChecks(endpoint, outputFile, wasmCodeFile) {
	console.log(`Generating setCodeWithoutChecks from RPC endpoint: ${endpoint} to outputFile: ${outputFile}, codeFile: ${wasmCodeFile}`);
	const wasmCode = fs.readFileSync(wasmCodeFile).toString('hex');
	connect(endpoint)
		.then((api) => {
			const call = api.tx.system.setCodeWithoutChecks(wasmCode);
			writeHexEncodedBytesToOutput(call.method, outputFile);
			exit(0);
		})
		.catch((e) => {
...
switch (type) {
	...
	case 'set-storage':
		...
		break;
	case 'set-code-without-checks':
		setCodeWithoutChecks(rpcEnpoint, output, inputArgs[0]);
		break;
	case 'check':
