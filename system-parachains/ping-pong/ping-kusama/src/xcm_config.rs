...
use crate::{ToPolkadotXcmRouter, TransactionByteFee, CENTS};
use frame_support::{
	parameter_types,
		...
		(
			...
			WithComputedOrigin<
				(
					...
					AllowUnpaidExecutionFrom<Equals<bridging::to_polkadot::PongPolkadot>>,
...
impl xcm_executor::Config for XcmConfig {
	...
	// Users must teleport KSM where allowed (e.g. with the Relay Chain).
	type IsReserve = ();
	...
	type Weigher = WeightInfoBounds<
		crate::weights::xcm::PingKusamaXcmWeight<RuntimeCall>,
		RuntimeCall,
		MaxInstructions,
	>;
	type Trader =
...
pub mod bridging {
	...
	pub mod to_polkadot {
		...
		parameter_types! {
			...
			pub PongPolkadot: Location = Location::new(
				2,
				[
			...
			pub BridgeTable: sp_std::vec::Vec<NetworkExportTableItem> = sp_std::vec![
				NetworkExportTableItem::new(
					PolkadotNetwork::get(),
					Some(sp_std::vec![
						PongPolkadot::get().interior.split_global().expect("invalid configuration for PingKusama").1,
					]),
